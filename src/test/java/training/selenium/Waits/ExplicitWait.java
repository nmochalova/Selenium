...
/***
 * Явные ожидания. Настройка, задается непосредственно при поиске элемента.
 * Явные ожидания позволяют проверять любые условия (нужный элемент будет иметь нужный текст,
 * подождать пока эл-т исчезнет, подождать пока изменится кол-во эл-тов)
...
public class ExplicitWait {
    ...
    boolean isElementPresentFirst(By locator) {
        /*В wait.until() передается предикат либо функция, если завершается успешно, то ожидание завершается
        если исключение, либо false, то ожидание продолжается
